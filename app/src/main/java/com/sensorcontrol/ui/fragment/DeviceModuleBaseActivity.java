...
import com.sensorcontrol.base.GosBaseActivity;
...
public class DeviceModuleBaseActivity extends GosBaseActivity{
     ...
     */
    // 数据点"data"对应的存储数据
    protected static byte[] data_data;
    ...
    protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
        ...
        if (dataMap.get("data") != null) {
            ...
            for (String dataKey : map.keySet()) {
                if (dataKey.equals(KEY_DATA)) {

                    data_data = (byte[]) map.get(dataKey);
                }
            }
        }
    ...
    protected GizWifiDeviceListener gizWifiDeviceListener = new GizWifiDeviceListener() {
        ...
        public void didSetSubscribe(GizWifiErrorCode result, GizWifiDevice device, boolean isSubscribed) {
            DeviceModuleBaseActivity.this.didSetSubscribe(result, device, isSubscribed);
        }

        /** 用于获取设备状态 */
        public void didReceiveData(GizWifiErrorCode result, GizWifiDevice device,
                                   java.util.concurrent.ConcurrentHashMap<String, Object> dataMap, int sn) {
            DeviceModuleBaseActivity.this.didReceiveData(result, device, dataMap, sn);
        }
        ...
        public void didGetHardwareInfo(GizWifiErrorCode result, GizWifiDevice device,
            ...
            DeviceModuleBaseActivity.this.didGetHardwareInfo(result, device, hardwareInfo);
        }

        /** 用于修改设备信息 */
        public void didSetCustomInfo(GizWifiErrorCode result, GizWifiDevice device) {
            DeviceModuleBaseActivity.this.didSetCustomInfo(result, device);
        }

        /** 用于设备状态变化 */
        public void didUpdateNetStatus(GizWifiDevice device, GizWifiDeviceNetStatus netStatus) {
            DeviceModuleBaseActivity.this.didUpdateNetStatus(device, netStatus);
        }
