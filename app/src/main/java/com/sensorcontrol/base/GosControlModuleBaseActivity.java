...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 * ===========================================================
	 */
	// 数据点"led_onoff"对应的标识名
	protected static final String KEY_LED_ONOFF = "led_onoff";
	// 数据点"pwm_led"对应的标识名
	protected static final String KEY_PWM_LED = "pwm_led";
	// 数据点"kz"对应的标识名
	protected static final String KEY_KZ = "kz";
	 ...
	 * ===========================================================
	 */
	// 数据点"pwm_led"对应seekbar滚动条补偿值
	protected static final int PWM_LED_OFFSET = 0;
	// 数据点"pwm_led"对应数据点增量值
	protected static final int PWM_LED_ADDITION = 0;
	// 数据点"pwm_led"对应数据点定义的分辨率
	protected static final int PWM_LED_RATIO = 1;
	 ...
	 * ===========================================================
	 */
	// 数据点"led_onoff"对应的存储数据
	protected static boolean data_led_onoff;
	// 数据点"pwm_led"对应的存储数据
	protected static int data_pwm_led;
	// 数据点"kz"对应的存储数据
	protected static byte[] data_kz;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				...
				if (dataKey.equals(KEY_PWM_LED)) {
			
					data_pwm_led = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_KZ)) {
			
					data_kz = (byte[]) map.get(dataKey);
				}
			}
		}
