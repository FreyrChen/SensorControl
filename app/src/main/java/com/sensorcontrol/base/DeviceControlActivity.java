...
public class DeviceControlActivity extends GosBaseActivity {
     ...
     */
    // 数据点"LED灯"对应的标识名
    protected static final String KEY_LED = "LED";
    // 数据点"PWM"对应的标识名
     ...
     * ===========================================================
     */
    // 数据点"PWM"对应seekbar滚动条补偿值
    protected static final int PWM_OFFSET = 0;
    // 数据点"PWM"对应数据点增量值
    protected static final int PWM_ADDITION = 0;
    // 数据点"PWM"对应数据点定义的分辨率
    protected static final int PWM_RATIO = 1;
     ...
     */
    // 数据点"LED灯"对应的存储数据
    protected static boolean data_LED;
    // 数据点"PWM"对应的存储数据
    protected static int data_PWM;
        ...
        if (dataMap.get("data") != null) {
            ...
            for (String dataKey : map.keySet()) {
                if (dataKey.equals(KEY_LED)) {
                    data_LED = (Boolean) map.get(dataKey);
                }
                if (dataKey.equals(KEY_PWM)) {

                    data_PWM = (Integer) map.get(dataKey);
                }
            }
        }
    ...
    protected GizWifiDeviceListener gizWifiDeviceListener = new GizWifiDeviceListener() {
        ...
        public void didSetSubscribe(GizWifiErrorCode result, GizWifiDevice device, boolean isSubscribed) {
            DeviceControlActivity.this.didSetSubscribe(result, device, isSubscribed);
        };

        /** 用于获取设备状态 */
        public void didReceiveData(GizWifiErrorCode result, GizWifiDevice device,
                                   java.util.concurrent.ConcurrentHashMap<String, Object> dataMap, int sn) {
            DeviceControlActivity.this.didReceiveData(result, device, dataMap, sn);
        };
        ...
        public void didGetHardwareInfo(GizWifiErrorCode result, GizWifiDevice device,
            ...
            DeviceControlActivity.this.didGetHardwareInfo(result, device, hardwareInfo);
        };

        /** 用于修改设备信息 */
        public void didSetCustomInfo(GizWifiErrorCode result, GizWifiDevice device) {
            DeviceControlActivity.this.didSetCustomInfo(result, device);
        };

        /** 用于设备状态变化 */
        public void didUpdateNetStatus(GizWifiDevice device, GizWifiDeviceNetStatus netStatus) {
            DeviceControlActivity.this.didUpdateNetStatus(device, netStatus);
        };
